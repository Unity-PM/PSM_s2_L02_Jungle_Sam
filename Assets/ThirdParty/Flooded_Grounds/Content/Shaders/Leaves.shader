Shader "JungleSam/Tree Creator Leaves URP"
{
    Properties
    {
        ...
        // Legacy Tree Creator properties.
        _TranslucencyColor ("Translucency Color", Color) = (0.5,0.6,0.25,1)
        _TranslucencyViewDependency ("Trans. View Dependency", Range(0,1)) = 0.7
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                float backLight = saturate(dot(-finalNormalWS, mainLight.direction));

                float translucency = backLight * _TranslucencyViewDependency;
                float3 translucencyColor = _TranslucencyColor.rgb * translucency;

                float shadowFactor = lerp(1.0, ndotl, _ShadowStrength);

                float3 color = albedoSample.rgb;
                color *= (0.35 + ndotl * 0.65) * mainLight.color;
                color += translucencyColor * 0.35;
                color *= shadowFactor;
